...
找到名称类似 `March7thAssistant_vx.x.x_full.zip` 的文件，点击下载。
...
找到下载完毕的名称类似 `March7thAssistant_vx.x.x_full.zip` 的文件，右键选择 "全部解压缩"。
...
成年女性模型（攻击为远程无位移）跑图最佳，如：艾丝妲、三月七，近战角色跑图会出现异常。
