...
1. まずゲームのデフォルトキーを変更していないか確認してください。特に自動戦闘（`V`）、インタラクト（`F`）、移動（`WASD`）が重要です。変更しているなら、まずデフォルトに戻してテストしてください。
...
**OpenVINO は一部の PC でメモリ使用量が継続的に増加することがあります**。アシスタントは定期的に推論エンジンを作り直してメモリを解放し、

利用可能な物理メモリが 4GB を下回ると自動で ONNXRuntime CPU に戻します。ただし、マシン自体のメモリが少ない場合は大きな変動が残ることがあります。
