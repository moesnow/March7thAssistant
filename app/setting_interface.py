...
from .card.comboboxsettingcard2 import (
    ComboBoxSettingCard2,
    ComboBoxSettingCardUpdateSource,
    ComboBoxSettingCardLog,
)
from .card.switchsettingcard1 import (
    SwitchSettingCard1,
    SwitchSettingCardNotify,
    StartMarch7thAssistantSwitchSettingCard,
    ...
    SwitchSettingCardCloudGameStatus,
)
from .card.rangesettingcard1 import RangeSettingCard1
from .card.pushsettingcard1 import (
    PushSettingCardInstance,
    PushSettingCardInstanceChallengeCount,
    ...
    PushSettingCardTeamWithSwap,
    PushSettingCardPowerPlan,
)
from .card.timepickersettingcard1 import TimePickerSettingCard1
from .card.expandable_switch_setting_card import (
    ExpandableSwitchSettingCard,
    ExpandableComboBoxSettingCardUpdateSource,
    ExpandablePushSettingCard,
    ExpandableComboBoxSettingCard,
    ExpandableComboBoxSettingCard1,
    ExpandableSwitchSettingCardEchoofwar,
)
...
class SettingInterface(ScrollArea):
    ...
    def __initWidget(self):
        ...
        scroller_props.setScrollMetric(
            QScrollerProperties.ScrollMetric.OvershootDragDistanceFactor, 0.05
        )
        scroller_props.setScrollMetric(
            QScrollerProperties.ScrollMetric.OvershootScrollDistanceFactor, 0.05
        )
        scroller_props.setScrollMetric(
            QScrollerProperties.ScrollMetric.DecelerationFactor, 0.5
        )
    ...
    def __initCard(self):
        ...
        self.tpBeforeInstanceEnableCard = SwitchSettingCard1(
            FIF.LEAF, "清体力前传送至任意锚点", "", "tp_before_instance"
        )
        ...
        self.borrowEnableCard = ExpandableSwitchSettingCard(
            "borrow_enable", FIF.PIN, "启用使用支援角色", ""
        )
        ...
        self.borrowFriendsCard = PushSettingCardFriends(
            "修改", FIF.FLAG, "支援列表", "borrow_friends"
        )
        ...
        self.echoofwarRunTimeCard = PushSettingCardDate(
            "修改", FIF.DATE_TIME, "上次完成历战余响的时间", "echo_of_war_timestamp"
        )
        ...
        self.dailyEnableCard = ExpandableSwitchSettingCard(
            "daily_enable", FIF.CALENDAR, "启用每日实训", ""
        )
        ...
        self.dailyMemoryOneTeamCard = PushSettingCardTeam(
            "修改", FIF.FLAG, "回忆一队伍", "daily_memory_one_team"
        )
        self.lastRunTimeCard = PushSettingCardDate(
            "修改", FIF.DATE_TIME, "上次检测到完成日常的时间", "last_run_timestamp"
        )
        self.activityEnableCard = ExpandableSwitchSettingCard(
            "activity_enable", FIF.CERTIFICATE, "启用活动检测", None
        )
        ...
        self.rewardEnableCard = ExpandableSwitchSettingCard(
            "reward_enable", FIF.TRANSPARENT, "启用奖励领取", ""
        )
        self.dispatchEnableCard = SwitchSettingCard1(
            FIF.STOP_WATCH, "委托", None, "reward_dispatch_enable"
        )
        self.mailEnableCard = SwitchSettingCard1(
            FIF.MAIL, "邮件", None, "reward_mail_enable"
        )
        self.assistEnableCard = SwitchSettingCard1(
            FIF.BRUSH, "支援", None, "reward_assist_enable"
        )
        self.questEnableCard = SwitchSettingCard1(
            FIF.STOP_WATCH, "每日实训", None, "reward_quest_enable"
        )
        self.srpassEnableCard = SwitchSettingCard1(
            FIF.QUIET_HOURS, "无名勋礼", None, "reward_srpass_enable"
        )
        self.achievementEnableCard = SwitchSettingCard1(
            FIF.CERTIFICATE, "成就", None, "reward_achievement_enable"
        )

        # 兑换码奖励开关
        self.redemptionEnableCard = SwitchSettingCard1(
            FIF.BOOK_SHELF, "兑换码", None, "reward_redemption_code_enable"
        )

        self.assetEnableCard = ExpandableSwitchSettingCard(
            "asset_manager_enable", FIF.LIBRARY, "启用资产管理", ""
        )
        ...
        self.weeklyDivergentEnableCard = ExpandableSwitchSettingCard(
            "weekly_divergent_enable", FIF.VPN, "启用「差分宇宙」积分奖励", ""
        )
        ...
        self.universeFateCard = ComboBoxSettingCard2(
            "universe_fate", FIF.PIE_SINGLE, "命途（仅模拟宇宙生效）", "", texts=fates
        )
        ...
        self.fightEnableCard = ExpandableSwitchSettingCard(
            "fight_enable", FIF.BUS, "启用锄大地 (Fhoe-Rail)", ""
        )
        ...
        self.fightTeamEnableCard = SwitchSettingCardTeam(
            FIF.EDIT, "自动切换队伍", None, "fight_team_enable", "fight_team_number"
        )
        ...
        self.FightRunTimeCard = PushSettingCardDate(
            "修改", FIF.DATE_TIME, "上次运行锄大地的时间", "fight_timestamp"
        )
        ...
        self.forgottenhallEnableCard = ExpandableSwitchSettingCard(
            "forgottenhall_enable", FIF.SPEED_HIGH, "启用混沌回忆", ""
        )
        self.forgottenhallLevelCard = PushSettingCardEval(
            "修改", FIF.MINIMIZE, "关卡范围", "forgottenhall_level"
        )
        ...
        self.forgottenhallTeamsCard = PushSettingCardTeamWithSwap(
            FIF.FLAG, "混沌回忆队伍配置", "forgottenhall_team1", "forgottenhall_team2"
        )
        self.forgottenhallRunTimeCard = PushSettingCardDate(
            "修改", FIF.DATE_TIME, "上次运行混沌回忆的时间", "forgottenhall_timestamp"
        )

        self.purefictionEnableCard = ExpandableSwitchSettingCard(
            "purefiction_enable", FIF.SPEED_HIGH, "启用虚构叙事", ""
        )
        self.purefictionLevelCard = PushSettingCardEval(
            "修改", FIF.MINIMIZE, "关卡范围", "purefiction_level"
        )
        self.purefictionTeamsCard = PushSettingCardTeamWithSwap(
            FIF.FLAG, "虚构叙事队伍配置", "purefiction_team1", "purefiction_team2"
        )
        self.purefictionRunTimeCard = PushSettingCardDate(
            "修改", FIF.DATE_TIME, "上次运行虚构叙事的时间", "purefiction_timestamp"
        )

        self.ApocalypticEnableCard = ExpandableSwitchSettingCard(
            "apocalyptic_enable", FIF.SPEED_HIGH, "启用末日幻影", ""
        )
        self.ApocalypticLevelCard = PushSettingCardEval(
            "修改", FIF.MINIMIZE, "关卡范围", "apocalyptic_level"
        )
        self.ApocalypticTeamsCard = PushSettingCardTeamWithSwap(
            FIF.FLAG, "末日幻影队伍配置", "apocalyptic_team1", "apocalyptic_team2"
        )
        self.ApocalypticRunTimeCard = PushSettingCardDate(
            "修改", FIF.DATE_TIME, "上次运行末日幻影的时间", "apocalyptic_timestamp"
        )

        self.CloudGameGroup = SettingCardGroup("云崩铁设置", self.scrollWidget)
        ...
        self.cloudGameFullScreenCard = SwitchSettingCard1(
            FIF.FULL_SCREEN, "全屏运行", None, "cloud_game_fullscreen_enable"
        )
        ...
        self.browserTypeCard = ExpandableComboBoxSettingCard(
            ...
            {
                "集成（Chrome For Testing）": "integrated",
                "Chrome": "chrome",
                "Edge": "edge",
            },
        )
        ...
        self.browserLaunchArgCard = PushSettingCardEval(
            "修改", FIF.CODE, "浏览器启动参数", "browser_launch_argument"
        )
        ...
        self.gamePathCard = PushSettingCard("修改", FIF.GAME, "游戏路径", cfg.game_path)
        self.updateViaLauncherEnableCard = ExpandableSwitchSettingCard(
            "update_via_launcher", FIF.UPDATE, "通过启动器更新游戏【测试版】", ""
        )
        self.launcherPathCard = PushSettingCard(
            "修改", FIF.GAME, "米哈游启动器路径", cfg.launcher_path
        )
        ...
        self.ScriptPathCard = PushSettingCard(
            "修改", FIF.CODE, "脚本或程序路径(选择运行脚本时生效)", cfg.script_path
        )
        self.playAudioCard = SwitchSettingCard1(
            FIF.ALBUM, "声音提示", "任务完成后列车长唱歌提示帕！", "play_audio"
        )
        ...
        self.notifyTemplateCard = PushSettingCardNotifyTemplate(
            "修改", FIF.FONT_SIZE, "消息推送格式", "notify_template"
        )
        ...
        for key, _ in cfg.config.items():
            if key.startswith("notify_") and key.endswith("_enable"):
                ...
                notifyEnableCard = SwitchSettingCardNotify(
                    (
                        self.notifyLogoDict[notifier_name]
                        if notifier_name in self.notifyLogoDict
                        else FIF.MAIL
                    ),
                    self.tr(
                        f'启用 {notifier_name.capitalize()} 通知 {"（支持图片）"if notifier_name in self.notifySupportImage else ""}'
                    ),
                    notifier_name,
                    key,
                )
        ...
        self.hotkeyCard = SwitchSettingCardHotkey(
            FIF.SETTING, "修改按键", "配置秘技、地图、跃迁、停止任务等按键设置"
        )
        ...
        self.qqGroupCard = PrimaryPushSettingCard(
            "加入群聊", FIF.EXPRESSIVE_INPUT_ENTRY, "QQ群", ""
        )
        self.feedbackCard = PrimaryPushSettingCard(
            "提供反馈", FIF.FEEDBACK, "提供反馈", "帮助我们改进 March7thAssistant"
        )
        self.aboutCard = PrimaryPushSettingCard(
            "检查更新", FIF.INFO, "关于", "当前版本：" + " " + cfg.version
        )
        ...
        self.checkUpdateCard = SwitchSettingCard1(
            FIF.SYNC, "启动时检测更新", "", "check_update"
        )
        self.updatePrereleaseEnableCard = SwitchSettingCard1(
            FIF.TRAIN, "加入预览版更新渠道", "", "update_prerelease_enable"
        )
        ...
        self.mirrorchyanCdkCard = PushSettingCardMirrorchyan(
            "修改", FIF.BOOK_SHELF, "Mirror 酱 CDK", self.parent, "mirrorchyan_cdk"
        )
    ...
    def __initLayout(self):
        ...
        self.echoofwarEnableCard.addSettingCards([self.echoofwarRunTimeCard])
        ...
        self.currencywarsEnableCard.addSettingCards([self.currencywarsRunTimeCard])
        ...
        self.weeklyDivergentEnableCard.addSettingCards(
            [self.weeklyDivergentTypeCard, self.weeklyDivergentRunTimeCard]
        )
        ...
        self.updateViaLauncherEnableCard.addSettingCards(
            [self.launcherPathCard, self.updateGameTimeoutCard]
        )
        ...
        self.testNotifyCard.addSettingCards(
            [self.notifyLevelCard, self.notifyTemplateCard]
        )
        ...
        if sys.platform != "win32":
            ...
            self.cloudGameEnableCard.setDisabled(
                True
            )  # 在配置文件中强制启用，禁止用户修改
        ...
        self.addSubInterface(
            self.CurrencywarsGroup, "CurrencywarsInterface", "货币战争"
        )
        if sys.platform == "win32":
        ...
        self.addSubInterface(
            self.ImmortalGameGroup, "ImmortalGameInterface", "逐光捡金"
        )

        self.pivot.addItem(
            routeKey="verticalBar",
            text="|",
            onClick=lambda: self.pivot.setCurrentItem(
    ...
    def __connectSignalToSlot(self):
        ...
        self.afterFinishCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )

        self.githubCard.clicked.connect(
            self.__openUrl("https://github.com/moesnow/March7thAssistant")
        )
        self.qqGroupCard.clicked.connect(
            self.__openUrl("https://qm.qq.com/q/C3IryUWCQw")
        )
        self.feedbackCard.clicked.connect(
            self.__openUrl("https://github.com/moesnow/March7thAssistant/issues")
        )

        self.aboutCard.clicked.connect(lambda: checkUpdate(self.parent))

        # 连接可展开卡片的展开状态改变信号，在动画前调整 stackedWidget 高度
        self.borrowEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.buildTargetEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.dailyEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.activityEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.rewardEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.assetEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.currencywarsEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.fightEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.weeklyDivergentEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.universeEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.forgottenhallEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.purefictionEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.ApocalypticEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.updateViaLauncherEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.updateSourceCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.testNotifyCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.instanceTypeCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.echoofwarEnableCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.browserTypeCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
        self.browserHeadlessCard.expandStateChanged.connect(
            self.__onExpandableCardStateChanged
        )
    ...
    def __onScriptPathCardClicked(self):
        script_path, _ = QFileDialog.getOpenFileName(
            self, "脚本或程序路径", "", "脚本或可执行文件 (*.ps1 *.bat *.exe)"
        )
    ...
    def __onExpandableCardStateChanged(self, is_expanding: bool):
        ...
        if is_expanding:
            self.stackedWidget.setFixedHeight(
                self.stackedWidget.currentWidget().sizeHint().height() + card_weight
            )
        else:
            self.stackedWidget.setFixedHeight(
                self.stackedWidget.currentWidget().sizeHint().height()
