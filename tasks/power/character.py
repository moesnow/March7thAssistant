...
class Character:
    ...
    def borrow():
        if not cfg.borrow_enable:
            return True
        if not (("使用支援角色并获得战斗胜利1次" in cfg.daily_tasks and cfg.daily_tasks["使用支援角色并获得战斗胜利1次"]) or cfg.borrow_character_enable):
            return True
        ...
        try:
            ...
            if cfg.borrow_character_from:
                ...
                log.info("找不到指定用户名的支援角色，尝试按照优先级选择")
