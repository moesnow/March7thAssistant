...
def parse_args():
    ...
    optional.add_argument(
        "-l", "--list", action="store_true", help="列出所有可用的任务"
    )
    optional.add_argument(
        "-e",
        "--exit",
    ...
    optional.add_argument(
        "-S", "--no-silent", action="store_true", help="禁用静默模式，显示命令行窗口"
    )
                    ...
                    if _main_window is not None:
                        try:
                            _main_window.handle_external_activate(
                                task=msg.get("task"),
                                exit_on_complete=msg.get("exit", False),
                            )
                        except Exception:
                            pass
                    else:
...
if __name__ == "__main__":
    ...
    try:
        payload = json.dumps(
            {"action": "activate", "task": args.task, "exit": args.exit}
        ).encode("utf-8")
    except Exception:
        payload = b"ACTIVATE"
        ...
        for msg in _pending_messages:
            try:
                w.handle_external_activate(
                    task=msg.get("task"), exit_on_complete=msg.get("exit", False)
                )
            except Exception:
                pass
